...
export default async function PresupuestoPage({ params }: Props) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6"
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-sm text-center">
          ...
          <h1 className="font-playfair text-2xl font-bold text-[#1C1C2E] mb-2">Tu propuesta ha caducado</h1>
          <p className="text-gray-500 text-sm leading-relaxed mb-6">
            Esta oferta ya no está disponible, pero podemos prepararte una nueva propuesta actualizada.
          </p>
          <a href={`https://wa.me/34${wa}?text=Hola%2C%20mi%20presupuesto%20ha%20caducado%20y%20me%20gustar%C3%ADa%20renovarlo`}
            className="flex items-center justify-center gap-2 w-full text-white font-bold py-4 rounded-2xl mb-3"
            style={{ background: '#25D366' }}>
            <span>💬</span> Escríbenos por WhatsApp
          </a>
          <a href={`mailto:${email}?subject=Renovar%20presupuesto%20de%20viaje`}
            className="flex items-center justify-center gap-2 w-full border-2 border-[#E8445A] text-[#E8445A] font-bold py-3 rounded-2xl hover:bg-[#E8445A] hover:text-white text-sm">
            ✉️ {email}
          </a>
  ...
  // Cargar variantes del mismo grupo (si las hay)
  let variantes: { id: string; variante: string; hotel: string; precio_total: number }[] = []
  if (presupuesto.grupo_id) {
    const { data: grupo } = await supabase
      .from('presupuestos')
      .select('id, variante, hotel, precio_total')
      .eq('grupo_id', presupuesto.grupo_id)
  ...
  return <OfertaPage presupuesto={presupuestoCompleto} variantes={variantes} />
}
